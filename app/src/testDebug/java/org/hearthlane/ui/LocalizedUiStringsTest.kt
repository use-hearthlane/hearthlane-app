package org.hearthlane.ui

import androidx.test.core.app.ApplicationProvider
import org.hearthlane.R
import org.junit.Assert.assertEquals
import org.junit.Test
import org.junit.runner.RunWith
import org.robolectric.RobolectricTestRunner
import org.robolectric.annotation.Config

/**
 * Resource-level coverage for the UI strings that back the main screens
 * (titles, Settings, events, empty/error/loading states, accessibility text).
 * Qualifiers pin the simulated device locale so English (default) and
 * Portuguese (Brazil) are verified independently of the machine running the
 * tests. Dynamic data (camera names, zones, ids, URLs) is intentionally absent:
 * it is never localized.
 */
@RunWith(RobolectricTestRunner::class)
@Config(sdk = [34])
class LocalizedUiStringsTest {

    private val context = ApplicationProvider.getApplicationContext<android.content.Context>()

    @Test
    fun `english default provides english ui strings`() {
        assertEquals("Hearthlane", context.getString(R.string.home_title))
        assertEquals("Cameras", context.getString(R.string.home_section_cameras))
        assertEquals("Settings", context.getString(R.string.settings_screen_title))
        assertEquals("Server settings", context.getString(R.string.settings_title))
        assertEquals("Initial setup", context.getString(R.string.setup_title))
        assertEquals("Diagnostics", context.getString(R.string.diagnostics_title))
        assertEquals("Recent events", context.getString(R.string.recent_events_title))
        assertEquals("Event", context.getString(R.string.event_detail_title))
        assertEquals("Loading settings…", context.getString(R.string.settings_loading))
        assertEquals("Server", context.getString(R.string.settings_section_server))
        assertEquals("Server address", context.getString(R.string.settings_server_address))
        assertEquals("Remote access", context.getString(R.string.settings_section_access))
        assertEquals("Reconfigure remote access", context.getString(R.string.settings_remote_access_reconfigure))
        assertEquals("Connected", context.getString(R.string.settings_connection_connected))
        assertEquals("Connecting…", context.getString(R.string.settings_connection_connecting))
        assertEquals("Unavailable", context.getString(R.string.settings_connection_unavailable))
        assertEquals("About", context.getString(R.string.settings_section_about))
        assertEquals("No recent events.", context.getString(R.string.recent_events_empty))
        assertEquals("Could not load events.", context.getString(R.string.recent_events_error))
        assertEquals("In progress", context.getString(R.string.event_in_progress))
        assertEquals("Today", context.getString(R.string.events_today))
        assertEquals("Yesterday", context.getString(R.string.events_yesterday))
        assertEquals("View event", context.getString(R.string.event_view_detail))
        assertEquals("Retry", context.getString(R.string.retry_button))
        assertEquals("HH:mm", context.getString(R.string.event_time_format))
        assertEquals("MMM d, yyyy · HH:mm", context.getString(R.string.event_datetime_format))
    }

    @Test
    @Config(qualifiers = "pt-rBR")
    fun `pt-br locale provides portuguese ui strings`() {
        assertEquals("Hearthlane", context.getString(R.string.home_title))
        assertEquals("Câmeras", context.getString(R.string.home_section_cameras))
        assertEquals("Configurações", context.getString(R.string.settings_screen_title))
        assertEquals("Configurações do servidor", context.getString(R.string.settings_title))
        assertEquals("Configuração inicial", context.getString(R.string.setup_title))
        assertEquals("Diagnóstico", context.getString(R.string.diagnostics_title))
        assertEquals("Eventos recentes", context.getString(R.string.recent_events_title))
        assertEquals("Evento", context.getString(R.string.event_detail_title))
        assertEquals("Carregando configurações…", context.getString(R.string.settings_loading))
        assertEquals("Servidor", context.getString(R.string.settings_section_server))
        assertEquals("Endereço do servidor", context.getString(R.string.settings_server_address))
        assertEquals("Acesso remoto", context.getString(R.string.settings_section_access))
        assertEquals("Reconfigurar acesso remoto", context.getString(R.string.settings_remote_access_reconfigure))
        assertEquals("Conectado", context.getString(R.string.settings_connection_connected))
        assertEquals("Conectando…", context.getString(R.string.settings_connection_connecting))
        assertEquals("Indisponível", context.getString(R.string.settings_connection_unavailable))
        assertEquals("Sobre", context.getString(R.string.settings_section_about))
        assertEquals("Nenhum evento recente.", context.getString(R.string.recent_events_empty))
        assertEquals("Não foi possível carregar os eventos.", context.getString(R.string.recent_events_error))
        assertEquals("Em andamento", context.getString(R.string.event_in_progress))
        assertEquals("Hoje", context.getString(R.string.events_today))
        assertEquals("Ontem", context.getString(R.string.events_yesterday))
        assertEquals("Ver evento", context.getString(R.string.event_view_detail))
        assertEquals("Tentar novamente", context.getString(R.string.retry_button))
        assertEquals("HH:mm", context.getString(R.string.event_time_format))
        assertEquals("d 'de' MMMM 'de' yyyy · HH:mm", context.getString(R.string.event_datetime_format))
    }
}
