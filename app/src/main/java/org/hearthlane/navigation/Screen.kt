...
/** Why a navigation reached Setup; the app renders Setup after onboarding only
 *  for an explicit user action (Settings -> server / reconfigure), never for a
 *  connectivity/auth auto-route. */
object SetupRouteReasons {
    const val INITIAL_ONBOARDING = "INITIAL_ONBOARDING"
    const val REMOTE_RECONFIGURE_USER_ACTION = "REMOTE_RECONFIGURE_USER_ACTION"
    const val USER_SERVER_SETTINGS = "USER_SERVER_SETTINGS"
...
sealed interface Screen {
    ...
    /**
     * Family Locations map. Reached from the Home tile; polls while shown and
     * stops polling on exit.
     */
 ...
 *
 * Every navigation to [Screen.Setup] records the explicit [setupReason], so the
 * composition root can enforce the invariant that a completed onboarding is
 * never auto-opened into Setup: only a user-initiated reason renders it.
 */
class AppNavigation(initial: Screen = Screen.Home) {
    ...
    /** Why Setup was last pushed; null when Setup was never pushed. */
    var setupReason: String? = null
        private set

    /** Pushes [screen] onto the back stack. [reason] is recorded only for Setup. */
    fun navigateTo(screen: Screen, reason: String = SetupRouteReasons.UNKNOWN) {
