...
internal object EventFormat {
    ...
    /**
     * The calendar day (in [zone]) an epoch timestamp belongs to. Used as the
     * stable grouping key for the recent-events list.
     */
    fun dayKey(
        epochSeconds: Double,
        zone: ZoneId = ZoneId.systemDefault(),
    ...
    /**
     * Formats a date-only label for a day group ("August 27"-style). [pattern]
     * is the localized layout; [locale] resolves month names.
     */
    fun dayLabel(
        epochSeconds: Double,
        zone: ZoneId = ZoneId.systemDefault(),
        pattern: String = "MMMM d",
        locale: Locale = Locale.getDefault(Locale.Category.FORMAT),
    ): String =
        DateTimeFormatter.ofPattern(pattern, locale)
            .format(Instant.ofEpochSecond(epochSeconds.toLong()).atZone(zone))
