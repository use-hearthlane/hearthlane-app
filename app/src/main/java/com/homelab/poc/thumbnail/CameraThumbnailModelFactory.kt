...
/**
 * Builds the Coil model for a camera thumbnail without exposing transport or
 * getter details to the Home screen. The factory observes the shared connection
 * and picks the same getter that playback uses, so thumbnails follow the
 * local-first / Tailscale-fallback policy automatically.
 */
